...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, 'options' | 'localize' | 'formatLong'> = zhCN;
    ...
    <DayPicker
      ...
      components={{
        ...
        MonthCaption: ({ calendarMonth }) => {
          return (
            <div className="inline-flex gap-2 pt-0.5">
              <Select
              ...
              >
                <SelectTrigger className="bg-transparent dark:bg-transparent dark:hover:bg-transparent w-fit gap-1 border-none p-0">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {MONTHS.map((month) => (
  ...
  (
    ...
    const initHourFormat = {
      hour24:
        ...
        `yyyy/MM/dd HH:mm${!granularity || granularity === 'second' ? ':ss' : ''}`,
      hour12:
        displayFormat?.hour12 ??
        `yyyy/MM/dd hh:mm${!granularity || granularity === 'second' ? ':ss' : ''} a`,
    };
