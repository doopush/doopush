...
export function EditAppDialog({ app, open, onOpenChange, onSuccess }: EditAppDialogProps) {
  ...
  const onSubmit = async (data: EditAppFormData) => {
    try {
      ...
      // 处理图标上传
      let appIcon = app.app_icon || '' // 保持原有图标
      ...
      const updatedApp = await AppService.updateApp(app.id, updateData)
      
      // 如果编辑的是当前选中的应用，更新currentApp缓存
      ...
      toast.error((error as Error).message || '更新应用失败')
    } finally {
      setLoading(false)
    }
  }
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    <FormLabel className="text-base">启用应用</FormLabel>
