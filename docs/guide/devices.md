...
**数据来源**：在线态由 Gateway 在握手成功 / 连接断开时写入 Redis（`device_online:<token>`，TTL 90 秒），列表接口读取设备时再用 Redis 实时覆写 `is_online` 字段。即使 MySQL 中的状态因异常未及时更新，列表展示仍以 Redis 的实时态为准。
