...
::: tip 💡 提示
当前注册无邮箱验证流程，提交后即可登录。请妥善保存自己的用户名与密码。平台暂未上线邮箱找回密码；遗忘后需联系管理员通过受控脚本生成 bcrypt 哈希并重置，不能把明文密码直接写入数据库。
:::
...
1. 在应用列表里点击该应用行末尾的 **"⋮"** 操作菜单 → **"API密钥"**
2. 在弹出的对话框中点击 **"创建 API 密钥"** 按钮
3. 填写密钥名称（如 `生产环境密钥` 或 `测试密钥`）
4. 点击 **"创建"** 按钮
5. **重要**：立即复制并保存生成的完整 API 密钥，它只会显示一次
