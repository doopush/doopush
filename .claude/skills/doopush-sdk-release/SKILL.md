...
| `doopush-ios-sdk` | `COCOAPODS_TRUNK_TOKEN` | `auto-build-release.yml` 的 "Publish to CocoaPods Trunk" 步骤；来自一次性 `pod trunk register <邮箱>`（当前 owner `aipaw@live.cn`）后 `~/.netrc` 里的 token。⚠️ 配在公仓不是 monorepo；缺失则该步骤优雅跳过、不发 pod |
