...
| SDK | 源目录 | 公仓 | 发布渠道 |
|---|---|---|---|
| iOS | `sdk/ios/DooPushSDK/` | `doopush/doopush-ios-sdk` | GitHub Release（framework.zip + podspec）+ SwiftPM + CocoaPods Trunk（`pod 'DooPushSDK'`） |
...
| `docs/sdk/android-integration.md` | Android bump | "方式一：JitPack（推荐）" 段里 `implementation 'com.github.doopush:doopush-android-sdk:X.Y.Z'` 那行 |
...
| iOS CocoaPods | `https://trunk.cocoapods.org/api/v1/pods/DooPushSDK/versions/X.Y.Z` 返回 200；公仓 run 里 "Publish to CocoaPods Trunk" 步骤绿勾 | 同 iOS | `pod 'DooPushSDK', '~> X.Y'` 可解析；trunk owner 为 `aipaw@live.cn`。RN 的 iOS 端通过裸 `s.dependency 'DooPushSDK'` 依赖此 pod，所以**iOS 发版必须连带把 pod 发上 trunk，否则 RN 外部 consumer 的 `pod install` 会失败** |
...
| Android | `lib/build.gradle` → `SDK_VERSION` 与 `version`（在 publishing 块） | **三处都要改**：①`lib/build.gradle` 的 `SDK_VERSION`（BuildConfig 来源）②`lib/src/main/java/com/doopush/sdk/DooPushDevice.kt` 的 `const val SDK_VERSION`（独立硬编码，非读 BuildConfig）③`DooPushNetworking.kt` 里 User-Agent 串 `DooPush-Android-SDK/X.Y.Z`（硬编码）。漏②③→真机上报版本与产物不一致 |
...
| `doopush-ios-sdk` | `COCOAPODS_TRUNK_TOKEN` | `auto-build-release.yml` 的 "Publish to CocoaPods Trunk" 步骤；来自一次性 `pod trunk register <邮箱>`（当前 owner `aipaw@live.cn`）后 `~/.netrc` 里的 token。⚠️ 配在公仓不是 monorepo。该步骤**全程非致命**：token 缺失/过期/push 失败时只发 `::warning::` 注解并跳过，**不阻塞 GitHub Release/SwiftPM**。token 过期（trunk session 有有效期）时按注解提示重新 `pod trunk register` 刷新、更新此 secret，再手动重跑 workflow 补发 pod |
