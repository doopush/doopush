...
> 前提：Step 0 诊断已经通过 precheck（在 main + 工作区干净），并决定了要发哪些 SDK。
...
`docs/` 站点**不在** sync workflow 监听路径里——它不会被推到 SDK 公仓——但里面有硬编码版本号字段，跟用户体验直接相关。SDK bump 后必须按下表更新；改动用**独立 commit**，不要混进 SDK 的 release commit。
...
| `docs/sdk/react-native-integration.md` | RN 或 native bump | Prerequisites 两行（iOS / Android 原生 SDK 最低版本）、`npx expo install doopush-react-native-sdk@X.Y.Z`、`npm install github:...#vX.Y.Z` 兜底命令、页脚"基于 vX.Y.Z 与 ... 对齐"声明 |

`docs/.vitepress/config.ts` 里只有导航结构、没有版本号，发版**不动它**。
...
**JitPack 是 lazy build**，不会自动监听 GitHub tag。`auto-build-release.yml` 在 Release 创建后会自动 curl 一下 pom URL 触发；如果工作流没跑或 ping 失败，回退到手动访问 pom URL。

---

## 参考

下面都是参考材料。**正常发版流程不需要从头读到尾**——发版过程里特定问题出现时回查对应小节。
...
监听 `main` 分支 + 路径过滤。skill 强制在 main 上发版，push 后直接触发对应 path 的 sync workflow。
...
第一行是 subject 加 bump 标记，scope 用 § 标准发版步骤 列出的规约。subject 写"做了什么 + 为什么"，下面正文（可选）展开背景。
